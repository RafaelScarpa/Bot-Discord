from nextcord import Streaming, Embed

activity=Streaming(name="⭐CLIQUE AQUI!",url='https://www.youtube.com/watch?v=ldmckFxztzA')

#+comandos
LstCmds= Embed(color=0x00ff90,title='Comandos:',description=(
...
links=[
  Embed(title="Rafael Scarpa no YouTube",url="https://youtube.co/RafaelScarpa/live",color=0x00ff90),
  Embed(title="Avisos do Scarpa no Twitter",url="https://twitter.com/ScarpaYT",color=0x00ff90),
  Embed(title="Doações?! :eyes:",url="https://streamelements.com/rafaelscarpa/tip",color=0x00ff90),
  Embed(title="Website secreto?! :flushed:",url="https://Bot-Discord.rafaelscarpa.repl.co",color=0x00ff90)]
...
def gifAbr(i,n=False,u='Você',f=0,): 
  a= Embed(title=f.format(n,u)) if n else Embed()
  a= a.set_image(url=i)
  return a
...
hora = '**Novo horário:** Normalmente a live começa às 15:00(BRT/GMT-3) na sabado.'
...
def gifTap(i,n=False,u='Você',f=0): 
  a= Embed(title=f.format(n,u)) if n else Embed()
  a= a.set_image(url=i)
  return a
...
def ranking(scores):
  ...
  return Embed(color=0x00ff90,title='Ranking de Score:',description=rank)